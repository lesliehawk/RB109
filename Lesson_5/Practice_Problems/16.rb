...
# uniquely identify items...which may not all be created by the same system.
...
def uuid_generator
  arr = []
  32.times { |i| arr << [*('a'..'z'),*('0'..'9')].shuffle.sample }
  str = arr.join
 "#{str[0..7]}-#{str[8..11]}-#{str[12..15]}-#{str[16..19]}-#{str[20..32]}"
end
