...
def guess_number(max_number, max_attempts)
  ...
  loop do
    ...
    if guess == winning_number
      puts 'Yes! You win.'
      break # was missing this break out of the loop so another attempt isn't made
    else
      puts 'Nope. Too small.' if guess < winning_number
      puts 'Nope. Too big.'   if guess > winning_number

    #Try again:
    #guess_number(max_number, max_attempts) <== not necessary, always recusive, attempts = 1 always
    end
  end
  puts "Thanks for playing!" #Tells player game is over
end
