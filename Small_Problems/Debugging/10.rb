...
# A friend of yours wants to build a basic search engine
# and starts reading up on information extraction.
# He decides to try a simple implementation of TF-IDF,
# a measure of the information content a term holds
# for a particular document in a document collection.
# However, he just started learning to code
# and somehow his numbers do not match what is expected.
# He knows you're enrolled in an awesome developer school,
...
def idf(term, documents)
  #need to have one value in equation a float to get a float return value
  number_of_documents = documents.length.to_f
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }
  #Removing document1 returns a 0 value when term == "quantum"
  #Rhis creates a ZeroDivisionError when the Math.log code is run.
  #To work around this, a return of 0 condition is needed for terms not available.
